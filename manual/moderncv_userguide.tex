...
\lstset{%
  ...
  % frame=L,
  emph={
    cvitem, cventry, cvdoubleentry, cvdoubleitem, cvtripleitem, cvlistitem, cvlistdoubleitem, cvcolumns, moderncvstyle, moderncvcolor, makecvtitle,
...
If you instead prefer to build your document from scratch, the only required commands are \code{\\moderncvstyle} and \code{\\name} in the preamble (see their usage below) and \code{\\makecvtitle} at the beginning of your document.
...
\begin{lstlisting}
  ...
  \moderncvstyle{<style>}
\end{lstlisting}
\note A \code{\\moderncvstyle} must be chosen for the document to compile.
\code{\\moderncvcolor} is optional, but must be used before \code{\\moderncvstyle}.
...
All of the commands except for \code{\\name} are optional, so try out what you like and see what you need.

\paragraph{\code{\\name} \textnormal{(required)}}
